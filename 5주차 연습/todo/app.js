...
const app = {
    ...
    renderItem(item) {
        const li = document.createElement("li");//To-Do 항목 표시하는 li 엘리먼트 생성
        if (item.completed) {
            //항목 완료 여부에 따라 css 클래스 추가
            li.classList.add("completed");
        }

        const titleText = document.createElement("div"); //할일 제목 표시 div 엘리먼트
        titleText.textContent = `${item.title} - ${item.dueDate}`;//할 일 제목과 마감 기한 텍스트 설정
        titleText.classList.add("title"); //css 클래스 추가

        const toggleBtn = document.createElement("button");//완료 여부 토글하는 버튼 엘리먼트
        toggleBtn.textContent = "✔️"; //텍스트 설정 
        toggleBtn.addEventListener("click", () => {
            //클릭 이벤트 핸들러 등록
            todoList.toggleComplete(item);//항목의 완료 여부 토글
        ...
        li.appendChild(toggleBtn); //li 엘리먼트 toggleBtn 엘리먼트 추가
        li.appendChild(removeBtn);//li 엘리먼트 removeBtn 엘리먼트 추가
        li.appendChild(titleText);//li 엘리먼트 titleText 엘리먼트 추가
        this.todoList.appendChild(li);//To-Do 항목 리스트 엘리먼트 li 엘리먼트 추가
    },

//To-Do 항목 리스트 렌더링 메소드
    renderList() {
        this.todoList.innerHTML = "";//기존 To-Do 항목 리스트 엘리먼트 내용 삭제
        todoList.items.forEach((item) => {
            this.renderItem(item);//To-Do 항목 리스트 순회하며 항목 렌더링
        });
    },
    //초기화 메소드
    init() {
